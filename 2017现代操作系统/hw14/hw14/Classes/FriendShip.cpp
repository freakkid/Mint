...
void FriendShip::addPlayer() {
	...
	// 设置角色刚体属性
	auto physicsBody = PhysicsBody::createBox(player1->getContentSize(), PhysicsMaterial(20.0f, 0.0f, 1.0f));
...
void FriendShip::onKeyPressed(EventKeyboard::KeyCode code, Event* event) {
	...
	case cocos2d::EventKeyboard::KeyCode::KEY_RIGHT_ARROW:
		...
		if (code == EventKeyboard::KeyCode::KEY_LEFT_ARROW) {
			...
			if (LastPlayer1Press == 'D') {
				LastPlayer1Press = 'A';
				player1->setFlippedX(true);	// 人物动画左右对称
			}
		}
		else {
			IsPlayer1Right = true;
	...
	case cocos2d::EventKeyboard::KeyCode::KEY_UP_ARROW:
		// 跳 
		// 判断玩家处于速度为0的状态
		if (player1->getPhysicsBody()->getVelocity().getDistance(Vec2(0, 0)) - 0 < 0.01) {
			IsPlayer1Jump = true;
		}
		break;
