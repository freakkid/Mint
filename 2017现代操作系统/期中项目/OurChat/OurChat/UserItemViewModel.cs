...
namespace OurChat {
    class UserItemViewModel {
        ...
        private UserItemViewModel() {
            ...
            using (var statement = db.Prepare("SELECT * FROM UserItem")) {
                while (SQLiteResult.ROW == statement.Step()) {
                    this.allItems.Add(new UserItem((string)statement[0], (string)statement[1], new Uri((string)statement[2])));
                }
            }
        ...
        public bool Add(string username, string password, Uri avatar_uri) {
            ...
            UserItem temp = new UserItem(username, MD5(password), avatar_uri);

            this.allItems.Add(temp);
            var db = App.conn;
            using (var userItem = db.Prepare("INSERT INTO UserItem(Username, Password, Avataruri) VALUES (?, ?, ?)")) {
                userItem.Bind(1, temp.username);
                userItem.Bind(2, temp.password);
                userItem.Bind(3, temp.avatarUri.ToString());
                userItem.Step();
            }
            return true;
        }
