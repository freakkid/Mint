...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		while (!this.isCompleted && !this.openList.isEmpty()) {
			// 如果该节点即是目标节点
			if (this.openList.firstElement().equals(endJNode)) {
				this.isCompleted = true;
				this.calSolutionPath();
			}
			else {
				// 寻找目前节点可走、未访问的下一个节点
				Vector<JigsawNode> followJnodes = this.findFollowJNodes(this.currentJNode);
				// 第一个节点已搜索，删除
				this.openList.remove(0);
				// 添加新的未访问的节点
				for (JigsawNode followJnode : followJnodes) {
					this.openList.add(followJnode);
				}
				// 访问第一个未访问的节点
				this.currentJNode = this.openList.firstElement();
				this.closeList.add(this.currentJNode);
	...
	private void estimateValue(JigsawNode jNode) {
		int s = 2 * estimateValue1(jNode) + estimateValue2(jNode);
	...
	// 所有 放错位的数码与其正确位置的距离 之和(曼哈顿距离)
	private int estimateValue2(JigsawNode jNode) {
		int s = 0; // 距离之和
		int dimension = JigsawNode.getDimension();
		for(int index =1 ; index<= dimension*dimension; index++){
			for (int index1 = 1; index1 <= dimension*dimension && jNode.getNodesState()[index] != 0; index1++) {
				if(jNode.getNodesState()[index] == this.endJNode.getNodesState()[index1]) {
					int row = (index - 1) / dimension;
					int col = (index - 1) % dimension;
					int row1 = (index1 - 1) / dimension;
					int col1 = (index1 - 1) % dimension;
					s += Math.abs(row - row1) + Math.abs(col - col1);
					break;
				}
			}
