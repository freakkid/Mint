...
namespace Todos
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        public MainPage()
        {
            ...
            SqlLocalData.SqlData sqlData = new SqlLocalData.SqlData();
        ...
        private void DataRequested(DataTransferManager sender, DataRequestedEventArgs e)
        {
            ...
            request.Data.SetBitmap(Windows.Storage.Streams.RandomAccessStreamReference.CreateFromUri(
                                                            new Uri(Models.TodoItem.defalutImagePath)));
        }
        ...
        private void CreateButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Uri imageUri = new Uri(Models.TodoItem.defalutImagePath);  // 使用默认背景图片
                    this.ViewModel.AddTodoItem(titleTextBox.Text, detailsTextBox.Text, imageUri, newImageSource, dueDatePicker.Date);
                }
                else
                {
                    // 背景图片不变
                    this.ViewModel.UpdateTodoItem(ViewModel.SelectedItem.getId(), titleTextBox.Text, detailsTextBox.Text, 
                                                                    this.ViewModel.SelectedItem.imageUri, newImageSource, dueDatePicker.Date);
                    namedCreateButton();
                }
        ...
        private async void SearchButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (item.title.Contains(SearchText.Text))
                {
                    sb.AppendLine("Title: " + item.title + "Description: " + item.description + "Time: " + item.dueDate.ToString());
                }
            }
            var i = await new MessageDialog(sb.ToString()).ShowAsync();
        }
